...
uint8_t g_rgb_frame_buffer[CONFIG_MATRIX_ROWS][CONFIG_MATRIX_COLS] = {{0}};
...
void nvs_update_rgb_matrix(void)
{
    nvs_flush_rgb_matrix(true);
}

void nvs_update_rgb_matrix_default(void)
{
    printf("nvs_update_rgb_matrix_default\n");
...
void nvs_debug_rgb_matrix(void)
{
...
void rgb_matrix_reload_from_eeprom(void)
{
    ...
    nvs_init_rgb_matrix();
    nvs_debug_rgb_matrix(); // display current eeprom values
...
{
#if defined(RGB_MATRIX_ENABLE) && defined(RGB_MATRIX_SPLIT)
    for (uint8_t i = 0; i < CONFIG_MATRIX_LED_COUNT; i++) {
...
#if defined(RGB_MATRIX_FRAMEBUFFER_EFFECTS) && defined(CONFIG_ENABLE_RGB_MATRIX_TYPING_HEATMAP)
#    if defined(RGB_MATRIX_KEYRELEASES)
    if (!pressed)
#    else
    if (pressed)
...
#endif // defined(RGB_MATRIX_FRAMEBUFFER_EFFECTS) && defined(CONFIG_ENABLE_RGB_MATRIX_TYPING_HEATMAP)
}
...
{
    nvs_flush_rgb_matrix(false);
    // next task
    if (sync_timer_elapsed32(g_rgb_timer) >= RGB_MATRIX_LED_FLUSH_LIMIT) {
...
#if defined(RGB_MATRIX_LED_PROCESS_LIMIT) && RGB_MATRIX_LED_PROCESS_LIMIT > 0 && RGB_MATRIX_LED_PROCESS_LIMIT < CONFIG_MATRIX_LED_COUNT
#    if defined(RGB_MATRIX_SPLIT)
    ...
    if (limits.led_max_index > CONFIG_MATRIX_LED_COUNT) {
        limits.led_max_index = CONFIG_MATRIX_LED_COUNT;
    }
    uint8_t k_rgb_matrix_split[2] = RGB_MATRIX_SPLIT;
    if (is_keyboard_left() && (limits.led_max_index > k_rgb_matrix_split[0])) {
        limits.led_max_index = k_rgb_matrix_split[0];
    }
    if (!(is_keyboard_left()) && (limits.led_min_index < k_rgb_matrix_split[0])) {
        limits.led_min_index = k_rgb_matrix_split[0];
    }
#    else
    limits.led_min_index = RGB_MATRIX_LED_PROCESS_LIMIT * (iter);
    limits.led_max_index = limits.led_min_index + RGB_MATRIX_LED_PROCESS_LIMIT;
...
void rgb_matrix_toggle_eeprom_helper(bool write_to_eeprom)
{
    ...
    rgb_task_state = STARTING;
    nvs_flush_rgb_matrix(write_to_eeprom);
    printf("rgb matrix toggle [%s]: rgb_matrix_config.enable = %u\n", (write_to_eeprom) ? "EEPROM" : "NOEEPROM", rgb_matrix_config.enable);
}
