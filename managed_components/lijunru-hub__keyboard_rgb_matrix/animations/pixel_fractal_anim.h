...
#ifdef CONFIG_ENABLE_RGB_MATRIX_PIXEL_FRACTAL
RGB_MATRIX_EFFECT(PIXEL_FRACTAL)
#    ifdef RGB_MATRIX_CUSTOM_EFFECT_IMPLS

static bool PIXEL_FRACTAL(effect_params_t* params)
{
#        if CONFIG_MATRIX_COLS < 2
#            define MID_COL 1
#        else
#            define MID_COL CONFIG_MATRIX_COLS / 2
#        endif
    static bool     led[CONFIG_MATRIX_ROWS][MID_COL];
    ...
    if (g_rgb_timer > wait_timer) {
        ...
        for (uint8_t h = 0; h < CONFIG_MATRIX_ROWS; ++h) {
            ...
            for (uint8_t l = 0; l < MID_COL - 1; ++l) {
                if (led[h][l]) {
                    ...
                    rgb_matrix_set_color(g_led_config.matrix_co[h][CONFIG_MATRIX_COLS - 1 - l], rgb.r, rgb.g, rgb.b);
                } else {
                    rgb_matrix_set_color(g_led_config.matrix_co[h][l], 0, 0, 0);
                    rgb_matrix_set_color(g_led_config.matrix_co[h][CONFIG_MATRIX_COLS - 1 - l], 0, 0, 0);
                }
                led[h][l] = led[h][l + 1];
            }
            ...
            if (led[h][MID_COL - 1]) {
                ...
                rgb_matrix_set_color(g_led_config.matrix_co[h][CONFIG_MATRIX_COLS - MID_COL], rgb.r, rgb.g, rgb.b);
            } else {
                rgb_matrix_set_color(g_led_config.matrix_co[h][MID_COL - 1], 0, 0, 0);
                rgb_matrix_set_color(g_led_config.matrix_co[h][CONFIG_MATRIX_COLS - MID_COL], 0, 0, 0);
            }
...
#endif     // CONFIG_ENABLE_RGB_MATRIX_PIXEL_FRACTAL
