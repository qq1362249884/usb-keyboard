...
#ifdef CONFIG_ENABLE_RGB_MATRIX_PIXEL_FLOW
...
{
    // LED state array
    static RGB led[CONFIG_MATRIX_LED_COUNT];
    ...
    if (params->init) {
        ...
        for (uint8_t j = 0; j < CONFIG_MATRIX_LED_COUNT; ++j) {
            led[j] = (random8() & 2) ? (RGB) {
                0, 0, 0
} : hsv_to_rgb((HSV) {
                random8(), random8_min_max(127, 255), rgb_matrix_config.hsv.v
            });
        }
    }
...
#endif     // CONFIG_ENABLE_RGB_MATRIX_PIXEL_FLOW
