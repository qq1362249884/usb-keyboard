#if defined(RGB_MATRIX_FRAMEBUFFER_EFFECTS) && defined(CONFIG_ENABLE_RGB_MATRIX_DIGITAL_RAIN)
...
bool DIGITAL_RAIN(effect_params_t* params)
{
    ...
    if (++drop > drop_ticks) {
        // reset drop timer
        drop = 0;
        for (uint8_t row = CONFIG_MATRIX_ROWS - 1; row > 0; row--) {
            for (uint8_t col = 0; col < CONFIG_MATRIX_COLS; col++) {
                // if ths is on the bottom row and bright allow decay
                if (row == CONFIG_MATRIX_ROWS - 1 && g_rgb_frame_buffer[row][col] == max_intensity) {
                    g_rgb_frame_buffer[row][col]--;
                }
...
#endif     // defined(RGB_MATRIX_FRAMEBUFFER_EFFECTS) && !defined(CONFIG_ENABLE_RGB_MATRIX_DIGITAL_RAIN)
