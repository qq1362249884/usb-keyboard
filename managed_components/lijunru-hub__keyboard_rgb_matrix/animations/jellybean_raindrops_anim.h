#ifdef CONFIG_ENABLE_RGB_MATRIX_JELLYBEAN_RAINDROPS
...
bool JELLYBEAN_RAINDROPS(effect_params_t* params)
{
    ...
    if (!params->init) {
        ...
        if (scale16by8(g_rgb_timer, qadd8(rgb_matrix_config.speed, 16)) % 5 == 0) {
            jellybean_raindrops_set_color(random8_max(CONFIG_MATRIX_LED_COUNT), params);
        }
    } else {
...
#endif     // CONFIG_ENABLE_RGB_MATRIX_JELLYBEAN_RAINDROPS
