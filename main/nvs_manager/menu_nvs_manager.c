...
esp_err_t menu_nvs_manager_save_all(MenuNvsManager_t* manager, uint8_t layer, bool ws2812_state) {
    ...
    ESP_LOGI(TAG, "Preparing to save menu config: layer=%d, ws2812_state=%d", layer, ws2812_state);
    ...
    if (open_err == ESP_OK) {
        ...
        if (verify_layer_err == ESP_OK && verify_ws_err == ESP_OK && 
            verify_layer == (uint32_t)layer && verify_ws_state == ws2812_state) {
            // 验证成功，不记录详细日志
        } else {
            ESP_LOGW(TAG, "Data verification after save: expected layer=%d, ws=%d; got layer=%" PRIu32 " (err=%s), ws=%d (err=%s)",
                    layer, ws2812_state, verify_layer, esp_err_to_name(verify_layer_err),
...
esp_err_t menu_nvs_manager_load_all(MenuNvsManager_t* manager, uint8_t* layer, bool* ws2812_state) {
    ...
    // 如果两个配置都找不到，返回ESP_OK但使用默认值
    if (layer_err == ESP_ERR_NOT_FOUND && ws_err == ESP_ERR_NOT_FOUND) {
        ESP_LOGW(TAG, "Both current layer and WS2812 state not found, using defaults");
    }
